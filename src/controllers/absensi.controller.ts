...
export class AbsensiController {
   ...
   */
  async uploadFingerprint(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.json(
        successResponse('Upload fingerprint berhasil diproses', result)
      );
    } catch (error) {
      next(error);
    }
   ...
   */
  async getKaryawanBelumAbsen(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.json(
        successResponse(
          'Data karyawan belum absen berhasil diambil',
          { karyawan, total: karyawan.length }
        )
      );
    } catch (error) {
      next(error);
  ...
  async createManual(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.status(201).json(
        successResponse('Absensi manual berhasil dibuat', absensi)
      );
    } catch (error) {
      next(error);
    }
   ...
   */
  async bulkCreateManual(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.status(201).json(
        successResponse('Bulk absensi manual berhasil diproses', result)
      );
    } catch (error) {
      next(error);
    }
  ...
  async findAll(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await absensiAgent.findAll(
        tanggalMulai,
        tanggalSelesai,
        validatedQuery.karyawanId,
        validatedQuery.statusKehadiran,
        isManual,
        page,
        limit
      );
  ...
  async findById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.json(
        successResponse('Data absensi berhasil diambil', absensi)
      );
    } catch (error) {
      next(error);
    }
  ...
  async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.json(
        successResponse('Absensi berhasil diupdate', absensi)
      );
    } catch (error) {
      next(error);
    }
  ...
  async delete(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.json(
        successResponse('Absensi berhasil dihapus')
      );
    } catch (error) {
      next(error);
    }
   ...
   */
  async getRingkasan(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const ringkasan = await absensiAgent.getRingkasan(
        new Date(tanggalMulai as string),
        new Date(tanggalSelesai as string),
        karyawanId as string | undefined
      );

      res.json(
        successResponse('Ringkasan absensi berhasil diambil', ringkasan)
      );
    } catch (error) {
      next(error);
    }
  ...
  async exportToExcel(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const buffer = await absensiAgent.exportToExcel(
        new Date(tanggalMulai as string),
        new Date(tanggalSelesai as string)
      );
