...
export class PengeluaranController {
    ...
    async findAll(req, res, next) {
        try {
            ...
            const keperluan = req.query.keperluan;
            const penerima = req.query.penerima;
            const tanggalMulai = req.query.tanggalMulai;
            const tanggalSelesai = req.query.tanggalSelesai;
            const page = req.query.page ? parseInt(req.query.page) : 1;
            const limit = req.query.limit ? parseInt(req.query.limit) : 20;
            const result = await pengeluaranAgent.findAll(itemId, kategori, keperluan, penerima, tanggalMulai, tanggalSelesai, page, limit);
            res.json(paginatedResponse('Data pengeluaran berhasil diambil', result.data, result.pagination));
        }
        catch (error) {
            next(error);
        }
     ...
     */
    async getRekapPengeluaran(req, res, next) {
        try {
            ...
            const rekap = await pengeluaranAgent.getRekapPengeluaran(kategori, tanggalMulai, tanggalSelesai, groupBy);
