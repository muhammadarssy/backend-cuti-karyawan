...
describe('PengeluaranAgent', () => {
    ...
    describe('findAll', () => {
        it('should get all pengeluaran with pagination', async () => {
            const result = await pengeluaranAgent.findAll(undefined, undefined, undefined, undefined, undefined, undefined, 1, 10);
            expect(result).toBeDefined();
            expect(result.data).toBeInstanceOf(Array);
            expect(result.pagination).toBeDefined();
        });
        it('should filter pengeluaran by itemId', async () => {
            const result = await pengeluaranAgent.findAll(itemId, undefined, undefined, undefined, undefined, undefined, 1, 10);
            expect(result.data.every((p) => p.itemId === itemId)).toBe(true);
        });
        it('should filter pengeluaran by keperluan', async () => {
            const result = await pengeluaranAgent.findAll(undefined, undefined, 'kantor', undefined, undefined, undefined, 1, 10);
            expect(result.data.length).toBeGreaterThan(0);
        });
        it('should filter pengeluaran by penerima', async () => {
            const result = await pengeluaranAgent.findAll(undefined, undefined, undefined, 'Umum', undefined, undefined, 1, 10);
            expect(result.data.length).toBeGreaterThan(0);
        });
    });
    describe('update', () => {
        ...
        it('should throw NotFoundError if pengeluaran not exists', async () => {
            const fakeId = '00000000-0000-0000-0000-000000000000';
            await expect(pengeluaranAgent.update(fakeId, { jumlah: 10 })).rejects.toThrow(NotFoundError);
        });
    });
    describe('delete', () => {
    ...
    describe('getRekapPengeluaran', () => {
        it('should get rekap by item', async () => {
            const rekap = await pengeluaranAgent.getRekapPengeluaran(undefined, '2026-01-01T00:00:00Z', '2026-01-31T23:59:59Z', 'item');
        ...
        it('should get rekap by keperluan', async () => {
            const rekap = await pengeluaranAgent.getRekapPengeluaran(undefined, '2026-01-01T00:00:00Z', '2026-01-31T23:59:59Z', 'keperluan');
            expect(rekap).toBeDefined();
            expect(rekap.rekap).toBeInstanceOf(Array);
        });
        it('should get rekap by penerima', async () => {
            const rekap = await pengeluaranAgent.getRekapPengeluaran(undefined, '2026-01-01T00:00:00Z', '2026-01-31T23:59:59Z', 'penerima');
            expect(rekap).toBeDefined();
            expect(rekap.rekap).toBeInstanceOf(Array);
        });
        it('should filter rekap by kategori', async () => {
            const rekap = await pengeluaranAgent.getRekapPengeluaran('ATK', '2026-01-01T00:00:00Z', '2026-01-31T23:59:59Z', 'item');
            expect(rekap).toBeDefined();
        });
    });
});
