...
export class PengeluaranAgent {
    ...
    async create(data) {
        ...
        if (item.stokSekarang < data.jumlah) {
            throw new ValidationError(`Stok tidak mencukupi. Stok tersedia: ${item.stokSekarang}, Jumlah diminta: ${data.jumlah}`);
        }
        // Parse tanggal
     ...
     */
    async findAll(itemId, kategori, keperluan, penerima, tanggalMulai, tanggalSelesai, page = 1, limit = 20) {
        ...
        if (tanggalMulai || tanggalSelesai) {
            where.tanggal = {};
            if (tanggalMulai)
                where.tanggal.gte = parseDate(tanggalMulai);
            if (tanggalSelesai)
                where.tanggal.lte = parseDate(tanggalSelesai);
        }
        if (kategori) {
            where.item = {
                kategori,
    ...
    async update(id, data) {
        ...
        if (data.jumlah !== undefined && data.jumlah !== oldPengeluaran.jumlah) {
            ...
            if (stockAfterAdjustment < 0) {
                throw new ValidationError(`Stok tidak mencukupi untuk perubahan ini. Stok tersedia: ${item.stokSekarang + oldPengeluaran.jumlah}, Jumlah diminta: ${newJumlah}`);
            }
        }
     ...
     */
    async getRekapPengeluaran(kategori, tanggalMulai, tanggalSelesai, groupBy = 'item') {
        ...
        if (groupBy === 'item') {
            ...
            const enrichedRekap = await Promise.all(rekap.map(async (r) => {
                const item = await prisma.item.findUnique({
                    where: { id: r.itemId },
