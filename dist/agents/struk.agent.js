...
export class StrukAgent {
    ...
    async create(data) {
        ...
        const allowedKategoriIds = new Set(budget.budgetKategori.map((bk) => bk.kategoriBudgetId));
        ...
        for (const item of data.items) {
            if (!allowedKategoriIds.has(item.kategoriBudgetId)) {
                throw new ValidationError(`Kategori budget untuk item "${item.namaItem}" tidak termasuk dalam budget bulan ini`);
            }
        }
        // Process setiap item
        for (const item of data.items) {
            ...
            const discountNominal = this.calculateDiscount(subtotal, item.discountType, item.discountValue);
     ...
     */
    async findAll(budgetId, tahun, bulan, page = 1, limit = 20) {
     ...
     */
    async getRekapByLabel(budgetId, tahun, bulan) {
     ...
     */
    async getRekapByKategori(budgetId, tahun, bulan) {
