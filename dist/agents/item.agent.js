...
export class ItemAgent {
     ...
     */
    async findAll(kategori, kode, nama, stokMenipis, page = 1, limit = 20) {
        ...
        if (stokMenipis === true) {
            filteredItems = items.filter((item) => item.stokSekarang <= item.stokMinimal);
        }
    ...
    async delete(id) {
        ...
        if (item.pembelian.length > 0 || item.pengeluaran.length > 0) {
            throw new BusinessLogicError('Item tidak bisa dihapus karena memiliki history transaksi');
        }
